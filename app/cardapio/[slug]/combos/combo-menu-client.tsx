...
export default function ComboMenuClient({ company, combos }: { company: Company; combos: Combo[] }) {
  ...
  return <main className="min-h-screen bg-[#fffaf5] pb-28">
    <header className="text-white" style={{ backgroundColor: company.primary_color || "#15803D" }}><div className="mx-auto max-w-6xl p-5 md:p-8"><a href={`/cardapio/${company.slug}`} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-white/90"><ArrowLeft size={17}/> Voltar ao cardápio</a><div className="flex items-center gap-4">{company.logo_url ? <img src={company.logo_url} alt="" className="h-16 w-16 rounded-2xl bg-white object-cover"/> : <div className="grid h-16 w-16 place-items-center rounded-2xl bg-white/20 text-3xl">🍔</div>}<div><p className="text-sm text-white/80">Combos MercadoFood</p><h1 className="text-3xl font-black">{company.name}</h1><p className="text-sm">{company.is_open ? "🟢 Aberto agora" : "🔴 Fechado"}</p></div></div></div></header>
