...
export default async function DriverSignup({ searchParams }: { searchParams: Promise<{ erro?: string; email?: string }> }) {
  ...
  return <main className="flex min-h-screen items-center justify-center bg-slate-950 p-4 text-white">
    <div className="w-full max-w-md rounded-3xl bg-slate-900 p-7">
      ...
      <form action={driverSignUp} className="mt-6 space-y-4">
        <div><label className="text-sm font-semibold">E-mail</label><input name="email" type="email" required defaultValue={q.email || ""} className="mt-1 w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3" /></div>
        <div><label className="text-sm font-semibold">Crie uma senha</label><PasswordInput name="password" minLength={6} required autoComplete="new-password" className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3" /></div>
