...
export default async function ClientesPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string; busca?: string }> }) {
  ...
  return <main className="space-y-6">
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h2 className="text-xl font-bold">Base de clientes</h2><p className="text-sm text-gray-500">Compras, gasto total e saldo de pontos. Use Editar para alterar o cadastro permanente.</p></div><form className="flex gap-2"><input name="busca" defaultValue={query.busca || ""} className="rounded-xl border px-3 py-2" placeholder="Buscar nome ou telefone" /><button className="rounded-xl bg-gray-900 px-4 py-2 text-sm font-semibold text-white">Buscar</button></form></div>
      <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead><tr className="border-b text-gray-500"><th className="py-3">Cliente</th><th>Contato</th><th>Pedidos</th><th>Total gasto</th><th>Pontos</th><th>Última compra</th><th>Marketing</th><th>Ações</th></tr></thead><tbody>{list.map(c=><tr key={c.id} className="border-b last:border-0"><td className="py-3"><strong>{c.name}</strong>{c.notes && <p className="max-w-xs truncate text-xs text-gray-500">{c.notes}</p>}</td><td>{c.phone}<br/><span className="text-xs text-gray-500">{c.email || "—"}</span></td><td>{c.total_orders}</td><td className="font-semibold">{money(c.total_spent)}</td><td><span className="rounded-full bg-orange-50 px-3 py-1 font-semibold text-orange-700">{c.loyalty_points} pts</span></td><td>{date(c.last_order_at)}</td><td>{c.marketing_consent ? "Autorizado" : "Não autorizado"}</td><td><a href={`/clientes/${c.id}/editar`} className="inline-flex rounded-lg border border-emerald-700 px-3 py-2 font-semibold text-emerald-800 hover:bg-emerald-50">Editar cadastro</a></td></tr>)}</tbody></table>{!list.length && <p className="rounded-xl bg-gray-50 p-6 text-gray-500">Nenhum cliente encontrado.</p>}</div>
    </section>
