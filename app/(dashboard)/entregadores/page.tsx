...
export default async function DriversPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string; novo?: string }> }) {
  ...
  const [{ data: drivers }, { data: readyOrders }, { data: activeDeliveries }, { data: notifications }, { data: unsettledDeliveries }, { data: payouts }, { data: driverAds }, { data: payoutAccounts }] = await Promise.all([
    ...
    supabase.from("deliveries").select("id, status, tracking_code, delivery_value, delivery_address, offered_at, picked_up_at, completed_at, problem_reason, created_at, drivers(name, phone, whatsapp), orders(order_number, customer_name, customer_phone)").eq("company_id", company.id).in("status", ["offered", "accepted", "to_store", "waiting_pickup", "delivering", "problem"]).order("created_at", { ascending: false }),
      ...
      <div className="mt-4 space-y-3">
        ...
        {readyOrders?.map((order) => <article key={order.id} className="rounded-xl border p-4">
          <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
            <div><p className="text-sm font-semibold text-orange-700">Pedido #{order.order_number}</p><h3 className="text-lg font-bold">{order.customer_name || "Cliente"}</h3><p className="mt-1 flex items-center gap-2 text-sm text-gray-500"><MapPin size={15}/>{String((order.delivery_address as any)?.street || "Endereço ainda não informado")}{(order.delivery_address as any)?.number ? `, ${(order.delivery_address as any).number}` : ""}</p>{(order.delivery_address as any)?.reference && <p className="mt-2 rounded-lg bg-orange-50 px-3 py-2 text-sm font-semibold text-orange-800">Ponto de referência: {(order.delivery_address as any).reference}</p>}<p className="mt-1 text-sm">Total: <strong>{money(order.total)}</strong></p></div>
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        {activeDeliveries?.map((delivery: any) => {
          ...
          return <article key={delivery.id} className="rounded-xl border p-4"><div className="flex items-start justify-between"><div><p className="text-sm font-semibold text-emerald-700">#{order?.order_number || "—"} • {delivery.tracking_code}</p><h3 className="font-bold">{order?.customer_name || "Cliente"}</h3><p className="text-sm text-gray-500"><Bike className="mr-1 inline" size={15}/>{driver?.name || "Motoboy"}</p></div><span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-orange-800">{delivery.status}</span></div>{destination?.reference && <p className="mt-3 rounded-lg bg-orange-50 px-3 py-2 text-sm font-semibold text-orange-800">Ponto de referência: {destination.reference}</p>}<dl className="mt-3 grid grid-cols-2 gap-2 rounded-lg bg-gray-50 p-3 text-xs"><div><dt className="text-gray-500">Atribuído</dt><dd className="font-semibold">{delivery.offered_at?new Date(delivery.offered_at).toLocaleString("pt-BR"):"—"}</dd></div><div><dt className="text-gray-500">Saiu para entrega</dt><dd className="font-semibold">{delivery.picked_up_at?new Date(delivery.picked_up_at).toLocaleString("pt-BR"):"—"}</dd></div>{delivery.problem_reason&&<div className="col-span-2 text-red-700"><dt>Problema</dt><dd className="font-semibold">{delivery.problem_reason}</dd></div>}</dl><p className="mt-3 text-sm">Corrida: <strong>{money(delivery.delivery_value)}</strong></p><div className="mt-3 flex flex-wrap gap-2"><WhatsAppButton phone={driver?.whatsapp || driver?.phone || ""} message={driverMessage} label="Avisar motoboy"/><WhatsAppButton phone={order?.customer_phone || ""} message={customerMessage} label="Enviar rastreamento"/></div></article>;
        })}
      </div>
    </section>
    <section className="rounded-2xl border bg-white p-5 shadow-sm">
