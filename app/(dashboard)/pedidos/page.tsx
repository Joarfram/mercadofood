...
export default async function PedidosPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  const [{ data: products, error: productsError }, { data: orders, error: ordersError }, { data: printers, error: printersError }] = await Promise.all([
    supabase.from("products").select("id,name,base_price,promotional_price,selling_mode,product_option_group_links!product_option_group_links_product_id_fkey(is_active,product_option_groups!product_option_group_links_group_id_fkey(id,name,min_selection,max_selection,free_selection,group_type,is_active,product_options(id,name,price_delta,max_quantity,is_active)))").eq("company_id", company.id).eq("availability_status", "available").eq("is_active", true).eq("selling_mode", "unit").eq("product_option_group_links.is_active",true).eq("product_option_group_links.product_option_groups.is_active",true).eq("product_option_group_links.product_option_groups.product_options.is_active",true).order("name"),
