...
export default async function EstoquePage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  const [{ data: ingredients }, { data: products }, { data: recipes }, { data: movements }] = await Promise.all([
    ...
    supabase.from("inventory_movements").select("id,movement_type,quantity,stock_after,notes,created_at,ingredient:ingredients(name,unit),product:products(name)").eq("company_id", company.id).order("created_at", { ascending: false }).limit(20),
  ]);
  ...
  return <main className="space-y-6">
    ...
    <section className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-bold">Últimas movimentações</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="border-b text-gray-500"><th className="py-3">Data</th><th>Item</th><th>Tipo</th><th>Quantidade</th><th>Saldo</th><th>Observação</th></tr></thead><tbody>{movements?.map((m:any)=>{const ing=Array.isArray(m.ingredient)?m.ingredient[0]:m.ingredient;const product=Array.isArray(m.product)?m.product[0]:m.product;const unit=ing?unitLabel[ing.unit]:"un";return <tr key={m.id} className="border-b last:border-0"><td className="py-3">{new Date(m.created_at).toLocaleString("pt-BR")}</td><td className="font-semibold">{ing?.name||product?.name||"Item removido"}</td><td>{movementLabel[m.movement_type]||m.movement_type}</td><td className={Number(m.quantity)<0?"text-orange-600":"text-emerald-700"}>{Number(m.quantity)>0?"+":""}{qty(m.quantity)} {unit}</td><td>{qty(m.stock_after)} {unit}</td><td className="text-gray-500">{m.notes||"—"}</td></tr>})}</tbody></table></div></section>
  </main>;
}
