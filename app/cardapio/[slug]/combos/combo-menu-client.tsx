...
export default function ComboMenuClient({ company, combos }: { company: Company; combos: Combo[] }) {
  ...
  function submit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      rememberPublicOrder(company.slug, { publicCode: result.data.public_code, orderNumber: result.data.order_number, total: Number(result.data.total), createdAt: new Date().toISOString() });
  ...
  return <main className="mf-public-menu min-h-screen bg-[#fffaf5] pb-28">
    ...
    <RecentOrderLink slug={company.slug}/>
