...
import { advanceOwnDelivery, confirmOwnPayout, driverSignOut, respondToDelivery, saveOwnPayoutProfile, setOwnAvailability } from "./actions";
...
export default async function DriverAppPage({ searchParams }: { searchParams: Promise<{ entrega?: string; tab?: string; erro?: string; sucesso?: string }> }) {
  ...
  const { data: driver } = await supabase.from("drivers")
    .select("id, company_id, name, email, phone, whatsapp, vehicle_plate, availability_status, default_delivery_value")
  ...
  const [{ data: delivery }, { data: completedDeliveries }, { data: payouts }, { data: driverAds }, { data: payoutAccount }] = await Promise.all([
    ...
    supabase.from("driver_payouts").select("id,amount,delivery_count,status,payment_method,payment_reference,paid_at,confirmed_at,created_at").eq("driver_id", driver.id).order("created_at", { ascending: false }).limit(20),
    supabase.from("media_assets").select("id,public_url,alt_text,sort_order").eq("company_id", driver.company_id).eq("entity_type", "company").eq("entity_id", driver.company_id).eq("kind", "gallery").order("sort_order").limit(5),
    supabase.from("driver_payout_accounts").select("payout_method,pix_key_type,pix_key,holder_name,city,bank_name,bank_branch,bank_account,bank_account_type").eq("driver_id",driver.id).maybeSingle(),
  ]);
  ...
  return <main className="min-h-screen bg-[#071821] px-4 pb-28 pt-4 text-white"><div className="mx-auto max-w-md space-y-4">
    ...
    {query.erro && <p className="rounded-2xl bg-red-500/15 p-4 text-sm text-red-200">{query.erro}</p>}
    {query.sucesso && <p className="rounded-2xl bg-emerald-500/15 p-4 text-sm text-emerald-200">{query.sucesso}</p>}
    ...
    {activeTab === "inicio" && <>
      ...
      {deliveryCard}
      {!delivery && <section className="rounded-3xl border border-orange-400/20 bg-gradient-to-br from-orange-500/15 to-emerald-500/10 p-5"><div className="flex items-center gap-3"><Gift className="text-orange-400"/><div><p className="text-xs font-bold uppercase tracking-wider text-orange-300">Benefícios do entregador</p><h2 className="font-bold">Ofertas de parceiros</h2></div></div>{driverAds?.length ? <div className="mt-4 flex snap-x gap-3 overflow-x-auto pb-2">{driverAds.map((ad) => <article key={ad.id} className="min-w-full snap-center overflow-hidden rounded-2xl bg-white text-slate-900"><img src={ad.public_url} alt={ad.alt_text || "Oferta para entregadores"} className="aspect-[16/7] w-full object-cover"/><p className="p-3 text-xs font-semibold text-slate-600">Publicidade • oferta do parceiro</p></article>)}</div> : <p className="mt-3 text-sm leading-6 text-slate-300">Cupons de combustível, manutenção e equipamentos aparecerão aqui.</p>}</section>}
    </>}
    ...
    {activeTab === "ganhos" && <><div><p className="text-sm font-semibold text-emerald-400">Seus resultados</p><h2 className="text-2xl font-bold">Ganhos e repasses</h2></div><section className="grid grid-cols-2 gap-3"><div className="rounded-2xl bg-emerald-600 p-5"><p className="text-sm text-white/75">Hoje</p><p className="mt-2 text-2xl font-bold">{money(dailyEarnings)}</p><p className="mt-1 text-xs text-white/70">{completedToday.length} entregas</p></div><div className="rounded-2xl border border-white/10 bg-white/5 p-5"><p className="text-sm text-slate-400">Últimos 7 dias</p><p className="mt-2 text-2xl font-bold">{money(weeklyEarnings)}</p><p className="mt-1 text-xs text-slate-400">{history.length} entregas</p></div></section><section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5"><h3 className="font-bold">Meus repasses</h3><div className="mt-4 space-y-3">{payouts?.map((payout) => <article key={payout.id} className="rounded-2xl bg-white/5 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-lg font-bold">{money(payout.amount)}</p><p className="text-xs text-slate-400">{payout.delivery_count} entregas • {shortDate(payout.created_at)}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${payout.status === "confirmed" ? "bg-emerald-500/20 text-emerald-300" : payout.status === "paid" ? "bg-orange-500/20 text-orange-200" : "bg-white/10 text-slate-300"}`}>{payout.status === "confirmed" ? "Confirmado" : payout.status === "paid" ? "Pago pela loja" : payout.status === "pending" ? "Aguardando pagamento" : "Cancelado"}</span></div>{payout.status === "paid" && <form action={confirmOwnPayout} className="mt-3"><input type="hidden" name="payoutId" value={payout.id}/><button className="w-full rounded-xl bg-emerald-600 py-3 font-bold">Confirmar que recebi</button></form>}{payout.payment_reference && <p className="mt-2 text-xs text-slate-400">Comprovante/referência: {payout.payment_reference}</p>}</article>)}{!payouts?.length && <p className="py-4 text-center text-sm text-slate-400">Nenhum repasse preparado pela loja.</p>}</div></section><section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-orange-300">Publicidade</p><h3 className="font-bold">Clube de vantagens</h3></div><Tag className="text-orange-400"/></div><div className="mt-4 grid gap-3"><div className="flex items-center gap-3 rounded-2xl bg-white/5 p-4"><Fuel className="text-emerald-400"/><div><b>Combustível</b><p className="text-xs text-slate-400">Cupons de postos parceiros.</p></div></div><div className="flex items-center gap-3 rounded-2xl bg-white/5 p-4"><Wrench className="text-orange-400"/><div><b>Manutenção da moto</b><p className="text-xs text-slate-400">Ofertas de oficinas patrocinadoras.</p></div></div></div></section></>}

    {activeTab === "perfil" && <><div><p className="text-sm font-semibold text-emerald-400">Minha conta</p><h2 className="text-2xl font-bold">Perfil e recebimento</h2></div><section className="rounded-3xl bg-white p-5 text-slate-900"><div className="flex items-center gap-4"><img src="/mercadofood-entrega-icon.svg" alt="" className="h-16 w-16 rounded-2xl"/><div><h3 className="text-xl font-bold">{driver.name}</h3><p className="text-sm text-slate-500">{driver.email || user.email}</p></div></div><dl className="mt-5 space-y-3 border-t pt-5 text-sm"><div className="flex justify-between gap-4"><dt className="text-slate-500">Telefone</dt><dd className="font-semibold">{driver.phone || "Não informado"}</dd></div><div className="flex justify-between gap-4"><dt className="text-slate-500">Placa</dt><dd className="font-semibold">{driver.vehicle_plate || "Não informada"}</dd></div><div className="flex justify-between gap-4"><dt className="text-slate-500">Valor padrão</dt><dd className="font-semibold">{money(driver.default_delivery_value)}</dd></div></dl></section><form action={saveOwnPayoutProfile} className="rounded-3xl bg-white p-5 text-slate-900"><h3 className="text-lg font-bold">Como quero receber</h3><p className="mt-1 text-xs text-slate-500">Estes dados ficam visíveis somente para você e para o proprietário da loja.</p><label className="mt-4 block text-sm font-semibold">Forma de recebimento<select name="payoutMethod" defaultValue={payoutAccount?.payout_method || "pix"} className="mt-1 w-full rounded-xl border px-3 py-3"><option value="pix">PIX</option><option value="bank">Conta bancária</option></select></label><label className="mt-3 block text-sm font-semibold">Nome do titular<input name="holderName" required defaultValue={payoutAccount?.holder_name || driver.name} className="mt-1 w-full rounded-xl border px-3 py-3"/></label><div className="mt-3 grid grid-cols-2 gap-3"><label className="text-sm font-semibold">Tipo da chave<select name="pixKeyType" defaultValue={payoutAccount?.pix_key_type || "random"} className="mt-1 w-full rounded-xl border px-3 py-3"><option value="cpf">CPF</option><option value="cnpj">CNPJ</option><option value="phone">Telefone</option><option value="email">E-mail</option><option value="random">Aleatória</option></select></label><label className="text-sm font-semibold">Cidade<input name="city" defaultValue={payoutAccount?.city || "ARACAJU"} className="mt-1 w-full rounded-xl border px-3 py-3"/></label></div><label className="mt-3 block text-sm font-semibold">Chave PIX<input name="pixKey" defaultValue={payoutAccount?.pix_key || ""} className="mt-1 w-full rounded-xl border px-3 py-3" placeholder="CPF, telefone, e-mail ou chave aleatória"/></label><details className="mt-4 rounded-xl bg-slate-50 p-3"><summary className="cursor-pointer text-sm font-semibold">Dados bancários opcionais</summary><div className="mt-3 grid gap-3"><input name="bankName" defaultValue={payoutAccount?.bank_name || ""} placeholder="Banco" className="rounded-xl border px-3 py-3"/><div className="grid grid-cols-2 gap-3"><input name="bankBranch" defaultValue={payoutAccount?.bank_branch || ""} placeholder="Agência" className="rounded-xl border px-3 py-3"/><input name="bankAccount" defaultValue={payoutAccount?.bank_account || ""} placeholder="Conta" className="rounded-xl border px-3 py-3"/></div><select name="bankAccountType" defaultValue={payoutAccount?.bank_account_type || "checking"} className="rounded-xl border px-3 py-3"><option value="checking">Conta corrente</option><option value="savings">Conta poupança</option><option value="payment">Conta de pagamento</option></select></div></details><button className="mt-4 w-full rounded-xl bg-emerald-700 py-3 font-bold text-white">Salvar dados de recebimento</button></form><InstallDriverApp/><section className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-xs leading-5 text-emerald-100"><div className="flex gap-3"><ShieldCheck className="shrink-0 text-emerald-400" size={21}/><p><b>Privacidade protegida:</b> seus dados de pagamento não aparecem para clientes ou funcionários comuns. O GPS permanece desligado fora das corridas.</p></div></section></>}
