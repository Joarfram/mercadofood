...
begin
  ...
  -- Se o conector caiu depois de assumir um trabalho, ele volta para a fila após 2 minutos.
...
begin
  ...
  set status = case when p_success then 'printed' else 'failed' end,
      ...
      next_attempt_at = case
        ...
        else now() + make_interval(secs => least(300, 15 * (2 ^ greatest(v_attempts - 1, 0))))
      end,
      updated_at = now()
  where id = p_job_id and printer_id = v_printer_id and status = 'printing';
