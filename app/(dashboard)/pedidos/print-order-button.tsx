...
type OrderItem = { product_name:string; quantity:number; selling_mode?:string|null; sale_quantity?:number|string|null; sale_unit?:string|null; unit_price:number|string; total_price:number|string; notes?:string|null; order_item_options?:Array<{ option_name:string; quantity:number; total_price:number|string }> };
...
function itemMeasure(item:OrderItem) {
  const quantity=Number(item.quantity||1);
  if(!item.selling_mode || item.selling_mode==="unit" || !item.sale_quantity || !item.sale_unit) return `${quantity}x`;
  const sale=Number(item.sale_quantity);
  const value=Number.isInteger(sale)?String(sale):sale.toLocaleString("pt-BR",{maximumFractionDigits:3});
  return item.selling_mode==="fixed_weight" && quantity>1 ? `${quantity}x ${value} ${item.sale_unit}` : `${value} ${item.sale_unit}`;
}
...
export function PrintOrderButton({ order, companyName, printer }: { order:PrintableOrder; companyName:string; printer:PrinterSettings|null }) {
  function printOrder() {
    ...
    const items = order.order_items.map((item)=>`<section class="item"><b>${clean(itemMeasure(item))} ${clean(item.product_name)}</b><span>${money(item.total_price)}</span>${item.order_item_options?.length ? `<small>${item.order_item_options.map(option=>`+ ${option.quantity || 1}x ${clean(option.option_name)}${Number(option.total_price)>0 ? ` (${money(option.total_price)})` : ""}`).join("<br>")}</small>`:""}${item.notes ? `<em>OBS.: ${clean(item.notes)}</em>`:""}</section>`).join("");
