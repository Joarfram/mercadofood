...
export default async function PedidosPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  const [{ data: products, error: productsError }, { data: orders, error: ordersError }, { data: printers, error: printersError }, { data: customers, error: customersError }, { data: addresses, error: addressesError }] = await Promise.all([
    ...
    supabase.from("orders").select("id, order_number, customer_name, customer_phone, status, service_type, subtotal, discount_amount, delivery_fee, total, coupon_code, loyalty_points_redeemed, payment_method, payment_status, change_amount, notes, delivery_address, created_at, order_items(product_name, quantity, unit_price, total_price, notes, order_item_options(option_name, quantity, total_price))").eq("company_id", company.id).or("status.in.(new,accepted,preparing,ready,out_for_delivery),payment_status.eq.pending").order("created_at", { ascending:false }).limit(100),
