-- Gestão Delivery Simples: consolida a medida real escolhida no carrinho,
-- recalcula o preço do item no servidor e aplica a baixa idempotente do estoque.
...
begin
  ...
  if not found then
    raise exception 'Pedido público inválido ou expirado para finalização.';
  end if;

  if jsonb_typeof(coalesce(p_items, '[]'::jsonb)) <> 'array' then
    raise exception 'Itens inválidos para finalização.';
  end if;
  ...
  loop
    ...
    where id = (v_payload_item->>'product_id')::uuid
    ...
    select count(*) + 1 into v_product_occurrence
    from jsonb_array_elements(coalesce(p_items, '[]'::jsonb)) with ordinality previous(value, ord)
    where previous.ord < (
      select min(current_item.ord)
      from jsonb_array_elements(coalesce(p_items, '[]'::jsonb)) with ordinality current_item(value, ord)
      where current_item.value = v_payload_item
    )
      and previous.value->>'product_id' = v_payload_item->>'product_id';

    -- Usa a próxima linha ainda não finalizada daquele produto. Isso suporta o mesmo produto
    -- repetido no carrinho com complementos diferentes sem depender de IDs vindos do navegador.
    ...
    if not found then
      raise exception 'Não foi possível relacionar um item do carrinho ao pedido.';
    end if;
    ...
    if coalesce(v_product.selling_mode, 'unit') = 'unit' then
      ...
      if v_sale_quantity is null or v_sale_quantity <= 0 then
        raise exception 'Informe o peso de %.', v_product.name;
      end if;
      -- Produto por peso representa uma única seleção de peso por linha.
      if v_line_quantity <> 1 then
        raise exception 'Produto por peso deve ser enviado em uma única seleção por item.';
      end if;
      v_unit_price := public.delivery_simple_weight_price(v_product.id, v_sale_quantity, v_sale_unit);
      ...
      if v_fixed_option is null then
        raise exception 'A embalagem selecionada para % não está disponível.', v_product.name;
      end if;
      v_unit_price := (v_fixed_option->>'price')::numeric;
    else
    ...
    set selling_mode = coalesce(v_product.selling_mode, 'unit'),
        ...
        total_price = round(v_unit_price * v_line_quantity, 2)
  ...
  where order_id = v_order.id and company_id = v_order.company_id;
...
comment on function public.delivery_simple_finalize_public_order(uuid,text,jsonb) is
  'Finaliza medidas e preços dos itens da Gestão Delivery Simples e aplica baixa idempotente do estoque.';