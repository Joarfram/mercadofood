-- Expõe banner e imagens promocionais no cardápio público e corrige mídias já enviadas.
update public.companies c
set banner_url = (
  select ma.public_url
  from public.media_assets ma
  where ma.company_id = c.id
...
where exists (
  ...
  where ma.company_id = c.id and ma.entity_type = 'company'
    and ma.entity_id = c.id and ma.kind = 'banner'
);
...
where exists (
  select 1 from public.media_assets ma
  where ma.company_id = c.id and ma.entity_type = 'company'
    and ma.entity_id = c.id and ma.kind = 'logo'
);
  ...
  select jsonb_build_object(
    ...
    'promotions', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'promotion_type', pr.promotion_type, 'image_url', pr.image_url
