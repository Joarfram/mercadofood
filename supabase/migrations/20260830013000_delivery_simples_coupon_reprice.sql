...
--
-- O create_public_order legado calcula cupom antes da etapa específica de peso. Esta função
-- substitui a finalização para manter desconto, resgate e pagamento coerentes com o subtotal real.
...
begin
  ...
  for v_payload_item in select value from jsonb_array_elements(coalesce(p_items, '[]'::jsonb)) loop
    ...
    -- Neste ponto a função cria somente a reserva. A baixa física acontece na confirmação.
  ...
  -- Recalcula o cupom contra o subtotal real final, não contra o preço-base do item.
  if v_order.coupon_id is not null then
    ...
    if v_coupon_removed_reason is not null then
      ...
      set coupon_id = null, coupon_code = null
      where id = v_order.id;
      v_discount := 0;
    else
  ...
  -- O pagamento pendente precisa acompanhar o total definitivo do pedido.
