...
export default async function ComplementosPage({ params, searchParams }: { params: Promise<{ productId: string }>; searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  return <main className="space-y-6">
    <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><Link href="/produtos" className="text-sm font-semibold text-emerald-700">← Produtos</Link><p className="mt-2 text-sm font-semibold text-orange-600">Complementos avançados</p><h1 className="text-3xl font-bold">{product.name}</h1><p className="text-gray-500">Preço base {money(product.base_price)}. Configure tamanho, sabores, bordas, ponto e acompanhamentos.</p></div></header>
    ...
    <section className="grid gap-5 xl:grid-cols-[380px_1fr]">
      <form action={createOptionGroup} className="h-fit rounded-2xl border bg-white p-5 shadow-sm">
        <input type="hidden" name="productId" value={product.id}/><h2 className="text-xl font-bold">Novo grupo</h2>
        <label className="mt-4 block text-sm font-semibold">Nome</label><input name="name" required placeholder="Ex.: Escolha o tamanho" className="mt-1 w-full rounded-xl border p-3"/>
        <label className="mt-3 block text-sm font-semibold">Descrição</label><input name="description" placeholder="Ex.: Selecione uma opção" className="mt-1 w-full rounded-xl border p-3"/>
        ...
        <button className="mt-5 w-full rounded-xl bg-emerald-700 py-3 font-bold text-white">Criar grupo</button>
      </form>

      <div className="space-y-4">
        {!groups?.length && <div className="rounded-2xl border bg-white p-8 text-center text-gray-500">Nenhum grupo criado.</div>}
        {groups?.map((group: any) => <article key={group.id} className="rounded-2xl border bg-white p-5 shadow-sm">
          ...
          <form action={createOption} className="mt-4 grid gap-2 border-t pt-4 md:grid-cols-[1fr_150px_120px_auto]"><input type="hidden" name="productId" value={product.id}/><input type="hidden" name="groupId" value={group.id}/><input name="name" required placeholder="Nova opção" className="rounded-xl border p-3"/><input name="priceDelta" type="number" step="0.01" defaultValue="0" placeholder="Acréscimo" className="rounded-xl border p-3"/><input name="maxQuantity" type="number" min="1" defaultValue="1" className="rounded-xl border p-3"/><button className="rounded-xl bg-orange-500 px-4 py-3 font-bold text-white">Adicionar</button></form>
        </article>)}
      </div>
    </section>
  </main>;
