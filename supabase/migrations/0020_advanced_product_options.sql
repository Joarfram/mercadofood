...
alter table public.product_option_groups
  add column if not exists group_type text not null default 'single'
    check (group_type in ('single','multiple','quantity')),
  add column if not exists free_selection integer not null default 0 check (free_selection >= 0);

alter table public.product_options
  add column if not exists max_quantity integer not null default 1 check (max_quantity >= 1);

-- Atualiza o cardápio público com os novos metadados.
...
begin
  select jsonb_build_object(
    'company', jsonb_build_object(
      ...
      'menu_message', c.menu_message, 'delivery_minimum', c.delivery_minimum,
          ...
          select jsonb_agg(jsonb_build_object(
            ...
            'option_groups', coalesce((
              select jsonb_agg(jsonb_build_object(
                ...
                'group_type', g.group_type, 'min_selection', g.min_selection,
                'max_selection', g.max_selection, 'free_selection', g.free_selection,
                'options', coalesce((
                  select jsonb_agg(jsonb_build_object(
                    'id', o.id, 'name', o.name, 'price_delta', o.price_delta, 'max_quantity', o.max_quantity
                  ) order by o.sort_order, o.name)
                  from product_options o where o.group_id = g.id and o.is_active = true
                ), '[]'::jsonb)
              ) order by g.sort_order, g.name)
              from product_option_groups g where g.product_id = p.id and g.is_active = true
            ), '[]'::jsonb)
          ) order by p.is_featured desc, p.name)
          from products p where p.category_id = cat.id and p.company_id = c.id and p.is_active = true and p.availability_status = 'available'
        ), '[]'::jsonb)
      ) order by cat.sort_order, cat.name)
      from categories cat where cat.company_id = c.id and cat.is_active = true
    ), '[]'::jsonb),
    'promotions', coalesce((
  ...
  from companies c where c.slug = p_slug and c.status = 'active' and c.menu_is_active = true;
  return result;
end;
$$;
