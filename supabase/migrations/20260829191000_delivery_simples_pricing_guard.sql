-- Cálculo centralizado no banco para impedir que o navegador defina o preço final.
...
begin
  ...
  v_minimum_grams := case when v_product.reference_unit = 'kg' then v_product.minimum_sale_quantity * 1000 else v_product.minimum_sale_quantity end;
  v_increment_grams := case when v_product.reference_unit = 'kg' then v_product.sale_increment * 1000 else v_product.sale_increment end;

  if v_requested_grams < v_minimum_grams then
...
revoke all on function public.delivery_simple_weight_price(uuid,numeric,text) from public;
grant execute on function public.delivery_simple_weight_price(uuid,numeric,text) to anon, authenticated;

comment on function public.delivery_simple_weight_price(uuid,numeric,text) is
  'Calcula e valida o preço de produto por peso usando somente dados persistidos no MercadoFood.';