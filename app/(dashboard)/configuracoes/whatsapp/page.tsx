import { Bot, CheckCircle2, ExternalLink, MessageCircle, ShieldCheck } from "lucide-react";
...
export default async function WhatsAppSettingsPage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  const { data: integration, error: integrationError } = await supabase.from("whatsapp_integrations").select("status,display_phone_number,chatbot_enabled,greeting_message,handoff_message,connected_at").eq("company_id", company.id).maybeSingle();
  ...
  return <main className="space-y-6">
    <header><p className="text-sm font-bold text-emerald-700">Atendimento integrado</p><h1 className="text-3xl font-black">WhatsApp e chatbot</h1><p className="mt-1 text-gray-500">Conecte o número da {company.name}, automatize as primeiras respostas e acompanhe as conversas.</p></header>
    ...
    <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
      <article className="rounded-2xl border bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-emerald-50 text-emerald-700"><MessageCircle/></div><div><h2 className="text-xl font-bold">Número do estabelecimento</h2><p className="text-sm text-gray-500">O próprio cliente autoriza o número pela Meta.</p></div></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${connected ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-600"}`}>{connected ? "Conectado" : "Não conectado"}</span></div>
      ...
      <article className="rounded-2xl border bg-[#063D2F] p-6 text-white shadow-sm"><ShieldCheck className="text-orange-400"/><h2 className="mt-4 text-xl font-bold">Conexão segura por empresa</h2><p className="mt-2 text-sm leading-6 text-emerald-50/80">Cada restaurante conecta apenas o próprio número. As credenciais ficam criptografadas, e nenhuma empresa consegue acessar conversas de outra.</p><a href="https://developers.facebook.com/docs/whatsapp/embedded-signup/" target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-orange-300">Documentação da Meta <ExternalLink size={15}/></a></article>
    </section>
