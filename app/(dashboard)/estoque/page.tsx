...
export default async function EstoquePage({ searchParams }: { searchParams: Promise<{ erro?: string; sucesso?: string }> }) {
  ...
  return <main className="space-y-6">
    ...
    <section className="grid gap-6 xl:grid-cols-3">
      ...
      <form action={saveRecipeItem} className="rounded-2xl border bg-white p-5 shadow-sm">
        <h2 className="text-xl font-bold">Ficha técnica</h2><p className="mt-1 text-sm text-gray-500">Informe quanto de um insumo é usado em uma unidade do produto.</p>
        <label className="mt-4 block text-sm font-semibold">Produto</label><select name="productId" required className="mt-1 w-full rounded-xl border px-3 py-3"><option value="">Selecione</option>{products?.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>
        <label className="mt-3 block text-sm font-semibold">Insumo</label><select name="ingredientId" required className="mt-1 w-full rounded-xl border px-3 py-3"><option value="">Selecione</option>{ingredients?.map(i=><option key={i.id} value={i.id}>{i.name} ({unitLabel[i.unit]})</option>)}</select>
        <label className="mt-3 block text-sm font-semibold">Quantidade por produto</label><input name="quantity" required type="number" min="0.001" step="0.001" className="mt-1 w-full rounded-xl border px-3 py-3" />
        <button className="mt-4 w-full rounded-xl border border-emerald-700 px-4 py-3 font-semibold text-emerald-800">Adicionar à ficha</button>
      </form>
    </section>

    <section className="grid gap-6 xl:grid-cols-[1.1fr_1fr]">
      <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-xl font-bold">Insumos</h2><span className="text-sm text-gray-500">{ingredients?.length || 0} itens</span></div><div className="mt-4 space-y-3">{!ingredients?.length && <p className="rounded-xl bg-gray-50 p-5 text-gray-500">Nenhum insumo cadastrado.</p>}{ingredients?.map(i=>{const isLow=Number(i.current_stock)<=Number(i.minimum_stock);return <article key={i.id} className="grid gap-2 rounded-xl border p-4 md:grid-cols-[1fr_auto_auto] md:items-center"><div><h3 className="font-bold">{i.name}</h3><p className="text-sm text-gray-500">Mínimo: {qty(i.minimum_stock)} {unitLabel[i.unit]} • Custo: {money(i.unit_cost)}/{unitLabel[i.unit]}</p></div><strong className={isLow?"text-orange-600":"text-emerald-700"}>{qty(i.current_stock)} {unitLabel[i.unit]}</strong><span className={`rounded-full px-3 py-1 text-xs font-semibold ${isLow?"bg-orange-50 text-orange-700":"bg-emerald-50 text-emerald-700"}`}>{isLow?"Estoque baixo":"Normal"}</span></article>})}</div></div>
