-- Gestão Delivery Simples: função centralizada para baixar estoque a partir do item do pedido.
-- Ainda não é ligada por trigger; será chamada pelo fluxo de criação do pedido quando o carrinho por peso estiver integrado.
...
begin
  ...
  if v_item.selling_mode = 'unit' then
    ...
    v_sale_quantity := v_item.sale_quantity;
...
comment on function public.delivery_simple_apply_order_item_stock(uuid) is 'Baixa idempotente do estoque de um item de pedido da Gestão Delivery Simples.';