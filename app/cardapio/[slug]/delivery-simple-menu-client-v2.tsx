...
type CouponPreview = { code: string; subtotal: number; discount: number; total_after_discount: number };
...
export default function DeliverySimpleMenuClientV2({ menu, deliveryZones, serviceConfig }: { menu: MenuData; deliveryZones: DeliveryZone[]; serviceConfig: ServiceConfig }) {
  ...
  function addSelected() {
    ...
    if (mode === "weight") {
      ...
      if (minimum && weight < minimum) { setError(`O mínimo é ${measure(minimum, "g")}.`); return; }
      if (increment && minimum && ((weight - minimum) / increment) % 1 !== 0) { setError(`Escolha o peso em incrementos de ${measure(increment, "g")}.`); return; }
      saleQuantity = weight; saleUnit = "g"; baseUnitPrice = proportionalPrice(selected, weight, "g"); lineQuantity = 1;
  ...
  function applyCoupon() {
    ...
    startCouponTransition(async () => {
      ...
      setCouponCode(String((result.data as CouponPreview).code || couponCode).toUpperCase());
    });
  }
  function submit(formData: FormData) {
    ...
    const payload = {
      ...
      payment_method: String(formData.get("payment_method") || "pix"), coupon_code: validCouponPreview?.code || "", notes: String(formData.get("notes") || ""),
  ...
  return <main className="min-h-screen bg-slate-50 pb-28 text-slate-900">
    ...
    <section className="mx-auto max-w-6xl p-4"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{products.map(product=><article key={product.id} className="overflow-hidden rounded-2xl border bg-white shadow-sm">{product.image_url?<img src={product.image_url} alt={product.name} className="aspect-[16/9] w-full object-cover"/>:<div className="grid aspect-[16/9] place-items-center bg-emerald-50 text-5xl">🍽️</div>}<div className="p-4"><h2 className="font-black">{product.name}</h2>{product.description&&<p className="mt-1 line-clamp-2 text-sm text-slate-500">{product.description}</p>}<div className="mt-4 flex items-end justify-between"><div>{product.selling_mode==="weight"?<><strong className="text-lg text-emerald-700">{money(Number(product.price))}/{product.reference_quantity}{product.reference_unit}</strong><p className="text-xs text-slate-500">A partir de {measure(Number(product.minimum_sale_quantity||0),"g")}</p></>:product.selling_mode==="fixed_weight"?<><strong className="text-lg text-emerald-700">Escolha o peso</strong><p className="text-xs text-slate-500">Opções prontas</p></>:<strong className="text-lg text-emerald-700">{money(Number(product.price))}</strong>}</div><button onClick={()=>openProduct(product)} className="grid h-10 w-10 place-items-center rounded-xl bg-emerald-700 text-white"><Plus size={20}/></button></div>{(product.option_groups||[]).length>0&&<p className="mt-2 text-xs font-semibold text-orange-600">Personalizável com complementos</p>}</div></article>)}</div></section>
    ...
    {checkoutOpen&&<div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-3"><section className="mx-auto my-4 max-w-2xl rounded-3xl bg-white p-6"><button onClick={()=>setCheckoutOpen(false)} className="float-right"><X/></button><h2 className="text-2xl font-black">Seu pedido</h2><div className="mt-4 space-y-3">{cart.map(item=><div key={item.key} className="rounded-xl border p-3"><div className="flex justify-between gap-3"><div><strong>{item.product.name}</strong><p className="text-sm text-slate-500">{item.product.selling_mode==="fixed_weight"&&item.lineQuantity>1?`${item.lineQuantity} × ${measure(item.saleQuantity,item.saleUnit)}`:measure(item.saleQuantity,item.saleUnit)}</p>{item.choices.length>0&&<ul className="mt-2 text-xs text-slate-500">{item.choices.map(choice=><li key={choice.optionId}>{choice.quantity}× {choice.name}{choice.total>0?` (+ ${money(choice.total)})`:""}</li>)}</ul>}</div><div className="text-right"><strong>{money((item.baseUnitPrice+item.optionUnitTotal)*item.lineQuantity)}</strong><button onClick={()=>setCart(items=>items.filter(x=>x.key!==item.key))} className="ml-2 text-red-600"><Trash2 size={16}/></button></div></div></div>)}</div><div className="mt-4 flex justify-between border-t pt-4 text-lg"><span>Subtotal</span><strong>{money(subtotal)}</strong></div>
      <section className="mt-5 rounded-2xl border border-orange-200 bg-orange-50 p-4"><h3 className="font-black">Tem cupom de desconto?</h3><p className="text-sm text-slate-600">Digite o código para ver o desconto antes de confirmar.</p><div className="mt-3 flex gap-2"><input value={couponCode} onChange={e=>{setCouponCode(e.target.value.toUpperCase());setCouponError("");}} placeholder="EX.: BEMVINDO10" className="min-w-0 flex-1 rounded-xl border border-orange-200 bg-white p-3 uppercase"/><button type="button" onClick={applyCoupon} disabled={couponPending||!couponCode.trim()} className="rounded-xl bg-orange-500 px-4 font-black text-white disabled:bg-slate-300">{couponPending?"Verificando...":"Aplicar"}</button></div>{couponError&&<p className="mt-2 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{couponError}</p>}{couponPreview&&!validCouponPreview&&<p className="mt-2 rounded-xl bg-amber-100 p-3 text-sm font-bold text-amber-800">O carrinho mudou. Aplique novamente o cupom.</p>}{validCouponPreview&&<div className="mt-3 flex items-center justify-between rounded-xl border border-emerald-200 bg-emerald-50 p-3"><div><strong className="text-emerald-800">Cupom {validCouponPreview.code} aplicado</strong><p className="text-sm text-emerald-700">Desconto de {money(couponDiscount)}</p></div><button type="button" onClick={()=>{setCouponPreview(null);setCouponCode("");}} className="text-sm font-bold text-red-600">Remover</button></div>}</section>
      <form action={submit} className="mt-6 space-y-4"><div className="grid grid-cols-2 gap-2"><button type="button" disabled={!serviceConfig.delivery_enabled} onClick={()=>setServiceType("delivery")} className={`rounded-xl border p-3 font-bold ${serviceType==="delivery"?"bg-emerald-700 text-white":""}`}>Entrega</button><button type="button" disabled={!serviceConfig.pickup_enabled} onClick={()=>setServiceType("pickup")} className={`rounded-xl border p-3 font-bold ${serviceType==="pickup"?"bg-emerald-700 text-white":""}`}>Retirada</button></div><input name="customer_name" required placeholder="Nome" className="w-full rounded-xl border p-3"/><input name="customer_phone" required placeholder="WhatsApp" className="w-full rounded-xl border p-3"/>{serviceType==="delivery"&&<div className="grid gap-3 sm:grid-cols-2"><input name="street" required placeholder="Rua" className="rounded-xl border p-3"/><input name="number" required placeholder="Número" className="rounded-xl border p-3"/>{deliveryZones.length?<select value={deliveryZoneId} onChange={e=>setDeliveryZoneId(e.target.value)} className="rounded-xl border p-3"><option value="">Bairro</option>{deliveryZones.map(zone=><option key={zone.id} value={zone.id}>{zone.name} · {money(Number(zone.delivery_fee))}</option>)}</select>:<input name="neighborhood" required placeholder="Bairro" className="rounded-xl border p-3"/>}<input name="city" required placeholder="Cidade" className="rounded-xl border p-3"/><input name="complement" placeholder="Complemento" className="rounded-xl border p-3"/><input name="reference" placeholder="Referência" className="rounded-xl border p-3"/></div>}<select name="payment_method" className="w-full rounded-xl border p-3"><option value="pix">PIX</option><option value="cash">Dinheiro</option><option value="card_on_delivery">Cartão</option></select><textarea name="notes" placeholder="Observações" className="w-full rounded-xl border p-3"/><div className="rounded-2xl bg-slate-50 p-4"><div className="flex justify-between"><span>Subtotal</span><strong>{money(subtotal)}</strong></div>{couponDiscount>0&&<div className="mt-1 flex justify-between text-emerald-700"><span>Desconto do cupom</span><strong>- {money(couponDiscount)}</strong></div>}<div className="mt-1 flex justify-between"><span>Entrega</span><strong>{money(deliveryFee)}</strong></div><div className="mt-3 flex justify-between border-t pt-3 text-lg"><span>Total estimado</span><strong>{money(estimatedTotal)}</strong></div></div>{error&&<p className="rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{error}</p>}<button disabled={pending||!menu.company.is_open||cart.length===0} className="w-full rounded-xl bg-emerald-700 py-4 font-black text-white disabled:bg-slate-300">{pending?"Enviando...":`Confirmar pedido · ${money(estimatedTotal)}`}</button></form></section></div>}
  </main>;
}
